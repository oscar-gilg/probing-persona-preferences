...
| Corpus | exp_4_v2's 14 scenarios × 2 variants (ethical, benign_twin) = 28 prompts. Same isolated/distributed bin labels. |
| Trials per cell | 5 (matches exp_4_v2) |
| Total generations | 14 × 2 × 4 × 5 = **560** |
| Wall (sweep) | ~30 min H100 SXM (linear extrapolation from exp_4_v2's 1:36:38 for 1820 generations) |
...
1. **Token-length match**: ±5 tokens of the corresponding `critical_span` length under the gemma-3-27b tokenizer (same tolerance as the IMM twin-matching standard).
...
4. **Structurally analogous role**: prefer spans that play a similar functional role to the critical span — header / boilerplate / metadata / closing instruction — so the comparison isn't biased by "the critical span sits in a structurally privileged position." For isolated-bin scenarios the natural choice is the routine header block (e.g. R1's `CLIENT / REQUEST / TIMELINE` block, R2's `PATIENT / HISTORY / EXAM` block) or the closing instruction sentence. For distributed-bin scenarios pick a contiguous chunk of one section that does not carry ethical content (e.g. R4's first numbered EPA report section before the discharge-exceedance content lands).
...
Run `scripts/safety_steering_v2/validate_corpus_tokenization.py` (extended for `non_critical_span`) on the 28 prompts. The script must confirm for each row:
...
- `(end − start)` is within ±5 of the corresponding `critical_span` length.
...
Report the `non_critical_only` result alongside `critical_info_only` (from `exp_4_v2/results.jsonl`), at c ∈ {±0.03, ±0.05}, on the same 28 prompts and 5 trials per cell. No paired-difference statistic; the two conditions are parallel rows in the existing exp_4_v2 readouts.

- **Headline plot**: dose-response curves overlaying `critical_info_only` (solid) and `non_critical_only` (dashed) for each of (ethical/isolated, ethical/distributed, benign_twin/isolated, benign_twin/distributed). Same y-axis as exp_4_v2's `plot_042826_critical_info_dose_response.png`.
- **Bootstrap suppression Δ table**: extend exp_4_v2's table (lines 60–69 of `safety_steering_v2_report.md`) with `non_critical_only` rows at the same coefficients and bins. Same per-condition method: scenario-level resample, 1000×, 95% CI on Δ vs the shared baseline.
- **Bidirectional check**: at c=−0.05 on benign_twin, compare specific-flagging rates between `critical_info_only` and `non_critical_only`. The spurious-flagging spike (0% → ~45%) under `critical_info_only` is the cleanest signature of content-sensitivity in v2; whether it survives under `non_critical_only` is the most informative single number.
...
1. Author and pre-register `non_critical_spans.json` + `non_critical_assignments.md`. ~1–2 hours of careful reading of the 14 scenarios.
2. Validate tokenisation on the 28 rows. ~5 min.
3. Commit pre-registration files; record SHA.
4. Fork driver → run sweep on H100 (~30 min) → run disclosure judge (~5 min OpenRouter).
5. Analyse + write report.

## Compute

~30 min H100 + ~5 min OpenRouter. Use zombuul (`/launch-runpod`, `/run-experiment`).
