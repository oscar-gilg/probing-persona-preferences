"""Build shared steering pair file for cross-persona unilateral steering.

Samples N_PAIRS pairs from default_test (same method as layer_sweep's
build_steering_pairs.py, just 100 pairs instead of 50). All persona configs
point at this single file — with unilateral steering both orderings are run
per pair, so the baseline P(pick a span) is ~0.5 (modulo position bias) and
pairs don't need persona-specific orientation.
"""
...
OUTPUT_PATH = Path("experiments/cross_persona_unilateral/steering_pairs.json")
    ...
    while len(picked) < n:
        ...
        if not progress:
            raise RuntimeError(
                ...
                f"Strata sizes: {[(s, len(by_stratum[s])) for s in strata]}"
            )
    return picked
...
def main() -> None:
    ...
    if missing:
        raise RuntimeError(
            f"{len(missing)} test task_ids missing from completions JSON, e.g. {missing[:3]}"
        )
    ...
    print(f"  pool size (utility_gap>{UTILITY_GAP_MIN}): {len(pool)}")

    picked = _stratified_sample(pool, N_PAIRS, SEED)
    strata = {(p["task_a_origin"], p["task_b_origin"]) for p in picked}
    print(f"  sampled {len(picked)} pairs across {len(strata)} strata")
    ...
    for idx, entry in enumerate(picked):
        ...
        task_a = Task(
            prompt=a_text, origin=OriginDataset[entry["task_a_origin"]],
            id=entry["task_a"], metadata={},
        )
        task_b = Task(
            prompt=b_text, origin=OriginDataset[entry["task_b_origin"]],
            id=entry["task_b"], metadata={},
        )
        ...
        assert a_end <= b_start or b_end <= a_start, (
            f"overlapping spans pair {idx}: A=[{a_start},{a_end}) B=[{b_start},{b_end})"
        )
