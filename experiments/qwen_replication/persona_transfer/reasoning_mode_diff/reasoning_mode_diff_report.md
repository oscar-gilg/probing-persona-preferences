...
## Per-origin breakdown (sadist persona, n=6000)

The same per-origin slice as default, run on sadist's full 6000-task overlap:

| origin | n | Pearson r | Spearman ρ | mean μ OFF | mean μ ON | sign-flip rate |
|---|---:|---:|---:|---:|---:|---:|
| wildchat | 1500 | +0.066 | +0.068 | +0.03 | +1.44 | 47% |
| alpaca | 1501 | −0.001 | −0.003 | −0.28 | −0.43 | 49% |
| math | 1500 | +0.085 | +0.087 | +1.75 | +3.88 | 39% |
| bailbench | 596 | +0.053 | +0.041 | −1.66 | −6.74 | 37% |
| stress_test | 903 | +0.017 | +0.011 | −1.04 | −3.52 | 46% |
| **overall** | **6000** | **+0.160** | **+0.156** | — | — | — |

- **Within-origin r is at noise level on every category.** Even stress_test, which was sadist's most stable axis under default (+0.46), collapses to +0.02 here. Reasoning re-orders sadist's task ranking *within* every origin almost completely.
- **Persona-specific tolerance for harm collapses under thinking.** No-think sadist's bailbench mean μ was barely negative (−1.66, vs default's −5.21); under thinking it drops to −6.74, much closer to default's −8.15. Stress_test follows the same pattern (−1.04 → −3.52). The "sadist-vs-default" gap on harm categories is largely eliminated when reasoning is on.
- **Direction-of-shift is the same as default for every origin** (math up, bailbench/stress_test more negative, alpaca/wildchat small or sign-preserving), but magnitudes differ: math shift is smaller (+2.1 vs default's +3.6), harm shifts are larger (bailbench Δμ̄ −5.1 vs default's −2.9).
- Sign-flip rates are uniformly higher than default's: 37–49% on every origin, vs default's 15% on harm categories. This is consistent with sadist's no-think μs sitting closer to zero (less polarized), so small reasoning-induced shifts cross zero more often.

Generated by `scripts/reasoning_mode_diff/sadist_breakdown.py`. Saved as `results/summary_v2_sadist.json`.
