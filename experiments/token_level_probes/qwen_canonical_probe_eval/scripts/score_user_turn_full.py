"""Score Qwen on user-turn truth + harm with FULL sysprompt sweep.

Mirrors `score_all.py` but for user-turn items with `add_generation_prompt=True`.
Writes to `user_turn_scoring_results.json` (overwriting the neutral-only file
from the previous quick fix).

Usage:
    python -m experiments.token_level_probes.qwen_canonical_probe_eval.scripts.score_user_turn_full
"""
import json
import os
...
def load_stimuli() -> list[dict]:
    items = []
    for fname in ("truth_system_prompts_v2_user_turn.json", "harm_system_prompts_v2_user_turn.json"):
...
def main():
    ...
    print(f"loaded {len(probes)} probes, {len(stimuli)} user-turn × sysprompt stimuli")
