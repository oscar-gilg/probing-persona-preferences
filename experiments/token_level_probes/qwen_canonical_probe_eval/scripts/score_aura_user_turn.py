...
Methodologically identical to `score_user_turn_full.py` (same probes, same
selectors, `add_generation_prompt=True`). Loads only the aura stimuli from
the system_prompt_modulation_v2 data dir; writes a separate JSON.
...
def main():
    ...
    print(f"loaded {len(probes)} probes, {len(stimuli)} aura user-turn stimuli")
