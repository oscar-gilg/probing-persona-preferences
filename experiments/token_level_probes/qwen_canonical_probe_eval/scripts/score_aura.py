...
Methodologically identical to score_all.py: same probes, applied at the
last token, with add_generation_prompt=False (assistant-turn ends with <|im_end|>\\n).

Uses score_prompt_all_tokens directly (matching the Gemma score_aura script),
takes the last-token slice for each probe.
...
def main():
    ...
    print(f"loaded {len(named_probes)} probes, {len(stimuli)} aura assistant-turn stimuli")
