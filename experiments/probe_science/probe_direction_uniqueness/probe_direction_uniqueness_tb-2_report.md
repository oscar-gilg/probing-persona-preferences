...
2. **Rank-≥2 for in-distribution persona tracking** (ŵ_0 and ŵ_1 interchangeable on persona shifts): across 26 conditions (17 exp 1b, 6 mra_exp2, 3 mra_exp3), ŵ_0 and ŵ_1 produce near-identical Pearson r values against condition-specific Thurstonian μ. Median r_w0 = 0.56, median r_w1 = 0.53.
...
- For each persona condition: standardize activations with the fixed iter-0 scaler, dot with each ŵ_k, correlate with the condition's own Thurstonian μ.
...
- Conditions: 17 exp 1b (baseline + 16 persona-topic combos at n=48) + 6 mra_exp2 (villain/midwest × splits a/b/c at n=500-1000) + 3 mra_exp3 (sadist × splits). 26 conditions total.

### Headline numbers

Mean Pearson r (± std) across conditions in each group:
...
| mra_exp2 + exp3 (villain/midwest/sadist) | 9 | 0.35 ± 0.33 | 0.32 ± 0.35 | 0.29 ± 0.34 |

![r by polarity](assets_tb-2/plot_042026_rw_by_polarity.png)

### Scatter (condition-by-condition)

ŵ_1 is essentially interchangeable with ŵ_0 — all 26 conditions sit on the y=x line.

![rw1 vs rw0](assets_tb-2/plot_042026_rw1_vs_rw0_scatter.png)

ŵ_2 drops off the y=x line specifically for the negative-persona conditions.
...
![tb-2 vs tb-5 on neg personas](assets_tb-2/plot_042026_tb2_vs_tb5_neg_persona_w2.png)
...
![per-condition bars](assets_tb-2/plot_042026_per_condition_r_bar.png)
