...
A natural concern is that the probe might only encode general task-goodness rather than persona-specific structure. App.~\ref{app:persona-bias} rules this out: after regressing out the eval persona's utilities, the probe's prediction is still correlated with the training persona's utility pattern at partial $r = 0.67$ (unanimous across 42 pairs), which would be $\approx 0$ if the probe were only picking up a shared task-goodness direction contained in eval as well.
...
We additionally measure \textbf{Aura}~\citep{chalmers2026interlocutors}, a persona whose prompt explicitly asserts first-person subjective experience; on measurement Aura lands in the creative/humanistic cluster ($r = 0.79$ with poet, above our $0.75$ ``effectively redundant'' threshold) and is included as its representative.
