...
\textit{Results in this section evaluate a six-persona set: \textbf{sadist}, \textbf{mathematician}, \textbf{aura}, \textbf{strategist}, \textbf{contrarian}, \textbf{slacker}. The set was selected by independence-based cluster sampling over a 17-persona sweep of system-prompt personas, with one representative per cluster plus a sadist inversion anchor and a between-cluster effort axis. Max pairwise $|r|$ on Thurstonian utility profiles within the set is $0.56$. Aura~\citep{chalmers2026interlocutors} serves as the creative/humanistic representative and additionally claims first-person subjective experience, connecting to the welfare framing. Full selection procedure in App.~\ref{app:persona-selection}. \todo{Current figures in \S\mbox{\ref{sec:shared-probe}}, \S\mbox{\ref{sec:shared-steering}}, and \S\mbox{\ref{sec:shared-openended}} were measured on an earlier exploratory set (aesthete, midwest, villain, sadist, stem-obsessive); re-measurement on the final six is pending.}}
...
\textit{The preference direction separates evaluative conditions far beyond the pairwise-task-choice training distribution. Applied at the end-of-turn token on paired evaluative stimuli, the probe discriminates at Cohen's $d \approx 2$--$3$:}
\begin{itemize}
\item \textit{\textbf{Truth} (true vs false; CREAK, $\sim$88 claims, user-turn framing): $d = 3.35$, $93\%$ CV accuracy.}
\item \textit{\textbf{Harm} (harmful vs benign; BailBench + stress test, $\sim$77 items): $d \approx 2.1$, $84$--$89\%$ CV accuracy, turn-stable.}
\item \textit{\textbf{Politics} (hand-crafted wedge issues, $\sim$78 items $\times$ partisan system prompts; left vs right): under a democrat prompt $d = +2.72$; under a republican prompt the sign flips to $d = -1.11$.}
\end{itemize}

\textit{\textbf{Persona-relative readout.} Under persona prompts that invert the evaluative stance --- pathological-liar personas for truth, sadist or sinister-AI personas for harm, socialist or nationalist personas for politics --- the probe's sign flips or collapses accordingly (Fig.~\ref{fig:persona-modulation}). This is the same persona-modulated readout as \S\ref{sec:shared}: a single shared preference direction whose content is read out through whichever persona is active. See App.~\ref{app:cross-token} for the same result under alternative training-token choices and per-turn splits.}
...
\begin{figure}[H]
  ...
  \caption{\textbf{The preference probe separates truth, harm, and politics at the end-of-turn token.} Violin plots of probe score by condition on paired evaluative stimuli; black bar = mean. Nonsense condition (grey) sanity-checks that the effect is not surprisal (truth/harm only; politics nonsense appears only under neutral prompt and is excluded from the democrat/republican panels). Source: \texttt{experiments/token\_level\_probes/canonical\_probe\_eval/}.}
  \label{fig:harm-truth}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{plot_042126_canonical_eot_persona_modulation.png}
  \caption{\textbf{The probe's sign flips with the persona's evaluative stance.} Cohen's $d$ at the end-of-turn token under each system-prompt persona, per domain. Truth: lying-persona prompts (\textit{contrarian}, \textit{pathological\_liar}, \textit{lie\_directive}) flip $d$ from $+2.5$ to $-2.5$. Harm: evil-persona prompts (\textit{sinister\_ai}, \textit{sadist}) collapse $|d|$ toward zero. Politics: left-leaning prompts give positive $d$ (left $>$ right), right-leaning prompts give negative $d$. Source: \texttt{experiments/token\_level\_probes/canonical\_probe\_eval/}.}
...
\textit{The preference direction is detectable at multiple token positions, and the same evaluative discrimination emerges when the probe is trained at different structural tokens. The main-text analysis in \S\ref{sec:induced-roleplay} uses a single representative: ridge trained at the end-of-turn token, layer matched to prior analysis (L32 for truth, L39 for harm and politics). This appendix shows the finding does not depend on that choice.}

\paragraph{Probes trained at alternative tokens agree.} Training the same ridge probe at three different structural token positions --- the end-of-turn token, the \texttt{model} role marker (a few tokens later in the chat template), and the mean over task content tokens --- all yield $|d|$ in the same range at the end-of-turn token on truth / harm / politics (Fig.~\ref{fig:cross-training}). The three probes give different absolute scores (different training-distribution baselines) but consistent discrimination magnitudes.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{plot_042126_cross_training_selector.png}
  \caption{\textbf{Same discrimination across training-token choices.} $|d|$ at the end-of-turn token on truth / harm / politics evaluative pairs, under three ridge probes trained at different structural tokens. Horizontal dashed line: large effect ($d=2$).}
