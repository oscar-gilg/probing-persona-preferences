"""Generate report figures for Qwen THINKING-mode persona probe transfer.

Same plotting code as `make_report_figures.py` but reads from the thinking results
dir and writes to the thinking assets dir.
"""
...
RESULTS = REPO / "experiments/qwen_replication/persona_transfer/probe_transfer/thinking/results"
...
def main() -> None:
    ...
    fig.suptitle("Probe transfer r — rows = train, cols = eval (THINKING mode)", fontsize=12)
    fig.tight_layout()
    ...
    fig.suptitle("Probe weight cosine (THINKING mode)", fontsize=12)
    fig.tight_layout()
    fig.savefig(ASSETS / f"plot_{DATE}_probe_cosine_grid.png", dpi=150)
    plt.close(fig)

    # --- Layer dependence outbound ---
    ...
    for i, sel in enumerate(SELECTORS):
        for j, layer in enumerate(LAYERS):
            ...
            ax.text(0.05, 0.95, f"{above}/{len(x)} above y=x", transform=ax.transAxes, va="top", fontsize=9)
    fig.suptitle("Transfer r vs utility-utility r (off-diagonal pairs, THINKING mode)", fontsize=12)
    fig.tight_layout()
    fig.savefig(ASSETS / f"plot_{DATE}_transfer_vs_utility_scatter.png", dpi=150)
    plt.close(fig)

    # --- Default-probe vs utility ---
    ...
    fig.suptitle("Default probe transfer vs default-vs-persona utility (THINKING mode)", fontsize=12)
    fig.tight_layout()
    ...
    fig.suptitle("Transfer asymmetry (THINKING mode)", fontsize=12)
    fig.tight_layout()
