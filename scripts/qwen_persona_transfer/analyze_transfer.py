"""Compute persona-transfer matrices for the Qwen-3.5-122B persona-sweep probes.

Loads the 14 trained probe sets (7 personas × 2 selectors, each with 3 layers)
and computes, for each (selector, layer) cell:

- Transfer matrix T (7×7): Pearson r between train-persona probe applied to
  eval-persona's test-split activations vs eval-persona's Thurstonian utilities.
- Probe-cosine matrix C (7×7): cosine similarity between probe weight vectors.

Plus one utility-utility matrix U (7×7): Pearson r between persona pairs'
Thurstonian utilities on the canonical test set.

Adapted from the Gemma analyze_transfer.py on research-loop/persona_probe_transfer.

Output schema (matches probe_transfer_spec.md):
- experiments/qwen_replication/persona_transfer/probe_transfer/results/
    transfer_<selector_tag>_L<layer>.npz: T (7,7), personas, selector, layer, probe_cosine
    utility_similarity.npz: U (7,7), personas, shared_task_ids
"""
...
def activations_path(persona: str, selector: str) -> Path:
    return REPO / f"activations/qwen35_122b/pref_{persona}_sweep/activations_{selector}.npz"
...
def main() -> None:
    ...
    for selector in SELECTORS:
        ...
        persona_activations: dict[str, dict[int, np.ndarray]] = {}
        ...
        for layer in LAYERS:
            ...
            off = transfer_r[off_mask].mean()
            print(f"saved {out.relative_to(REPO)}: diag={diag:.3f} off-diag={off:.3f}")
