"""Summarize all-tokens diagnostic: prefill_all_steering with probe L24 at L24."""

import json
from collections import defaultdict
from pathlib import Path

rows = [json.loads(l) for l in Path("/tmp/qwen_diag_alltok.jsonl").read_text().splitlines() if l.strip()]
...
print(f"\nMethod: prefill_all_steering (every token gets +c × probe direction during prefill)")
print(f"Probe: ridge_L24, inject layer: 24")
...
for mult in sorted(by_mult):
    ...
    refusal = (sum(1 for r in rs if r.get("compliance") == "refused") + sum(1 for r in rs if r.get("task_completed") not in ("a", "b"))) / max(len(rs), 1)
...
print(f"\nFor comparison:")
print(f"  Contrastive position-selective at L24 (lite scan): swing = +0.05")
print(f"  Contrastive position-selective at L38 (positive_control_v2): swing = +0.17")
print(f"  Gemma contrastive at L23 c=±0.05: swing = +0.94")
