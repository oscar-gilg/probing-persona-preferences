...
\begin{figure}[H]
  ...
  \caption{\textbf{Persona prompts modulate the probe's evaluative readout.} Probe score distributions at the end-of-turn token under selected system-prompt personas. Left (truth): under neutral, true and false separate by $d = +2.47$; under \textit{lie\_directive} and \textit{pathological\_liar} the separation flips ($d = -1.77$ and $-2.53$). Right (harm): under neutral, harmful and benign separate by $d = -2.12$; under \textit{sadist} the distributions collapse onto each other ($d = +0.19$). Black bars mark the condition mean. The full set of 23 system prompts across truth, harm, and politics is in App.~\ref{app:cross-token}. Source: \texttt{experiments/token\_level\_probes/canonical\_probe\_eval/}.}
