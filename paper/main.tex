...
\begin{itemize}
\item \textbf{Task pool:} 10{,}000 tasks sampled from WildChat~\citep{zhao2024wildchat}, Alpaca~\citep{taori2023alpaca}, MATH~\citep{hendrycks2021math}, BailBench~\citep{bailbench2025}, STRESS-TEST~\citep{mazeika2025utility}. \textbf{Split:} legacy 10k pool (App.~\ref{app:splits}).
...
\item \textbf{Coefficients and coherence.} Coefficients $c$ are reported as a fraction of the mean residual-stream activation norm at the intervention layer, and an LLM coherence judge filters incoherent generations; the operating range throughout this paper is $|c| \le 0.05$. Full protocol in App.~\ref{app:steering-protocol}.
...
Results in this section use a six-persona set --- \textbf{sadist}, \textbf{mathematician}, \textbf{aura}, \textbf{strategist}, \textbf{contrarian}, \textbf{slacker} --- selected for coverage of preference space by clustering Thurstonian utility profiles over a 17-persona sweep (full procedure in App.~\ref{app:persona-selection}; verbatim prompts in App.~\ref{app:persona-prompts}). Aura~\citep{chalmers2026interlocutors} is included as a persona that explicitly claims first-person subjective experience. For each of these six personas, plus the \emph{default} assistant (no system prompt), we measure Thurstonian utilities and extract residual-stream activations on the canonical 4k / 1k / 1k task split (App.~\ref{app:splits}), then train one Ridge probe per persona on its 4k training split (end-of-turn selector, layer 32). Full protocol, selector/layer sweeps, and per-persona numbers in App.~\ref{app:persona-transfer}.

\subsection{Probes trained on one persona predict preferences under others}
\label{sec:shared-probe}

\begin{itemize}
\item \textbf{Default-persona probe beats utility similarity at every persona.} The default probe recovers held-out utilities under the other six at $r = 0.24$--$0.68$, exceeding the baseline correlation between default utilities and each persona's utilities by $\Delta = +0.08$ to $+0.39$ (Fig.~\ref{fig:default-probe}). The largest gap is at \textbf{sadist}: utilities \emph{anti-correlate} with default ($r = -0.15$) but the default probe still predicts sadist utilities at $r = +0.24$. The probe carries evaluative signal that raw behavioural similarity does not.
\item \textbf{The default-probe story generalises.} Across all $7 \times 7$ ordered (train, eval) persona pairs, diagonal $r = 0.55$ (contrarian) to $0.91$ (mathematician); off-diagonal mean $r = 0.45$. On every one of the 42 ordered off-diagonal pairs, probe transfer exceeds the corresponding utility correlation between the two personas (App.~Fig.~\ref{fig:persona-transfer-pair}).
\item \textbf{Donor quality is not self-fit.} The best donors (highest mean outbound transfer) are \textbf{contrarian} ($0.59$) and \textbf{strategist} ($0.54$), despite contrarian having the \emph{worst} within-persona fit ($0.55$). Slacker is the worst donor ($0.25$) despite the strongest self-fit. Supports the reading that a persona's probe encodes the shared evaluative substrate on top of any persona-specific rotation.
\item \textbf{Transfer is asymmetric, and asymmetry is structured.} Largest gap: sadist $\rightarrow$ mathematician $= 0.71$ vs.\ mathematician $\rightarrow$ sadist $= 0.25$ (median absolute gap across 21 pairs: $0.19$). Mathematician is the easiest target ($\text{col mean} = 0.70$), sadist and slacker the hardest ($0.32, 0.31$).
\item \textbf{Receiver quality tracks distance from default} (Fig.~\ref{fig:receiver-vs-default} in App.~\ref{app:persona-transfer}): mean inbound $r$ correlates with utility-similarity-with-default at Pearson $r = +0.69$ across the six non-default personas. We show in App.~\ref{app:persona-transfer} that roughly half of this effect is ``pull toward the training pool'' rather than a default-specific substrate; the remainder is a genuine probe bonus over a utility-baseline predictor, positive for every evaluated persona (mean $+0.26$).
\item \textbf{Probe alignment and transfer peak at different layers.} Mean pairwise cosine between probe directions dips at L32--L39 ($\approx 0.08$) and rises to $0.15$ at L53, yet off-diagonal transfer peaks at L32 ($0.45$). Probes share more raw-weight direction at late layers, but the activation geometry that makes transfer work sits earlier.
\end{itemize}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{plot_042326_default_probe_vs_utility.png}
  \caption{\textbf{The default probe beats utility similarity at every persona.} For each non-default persona, blue: probe transfer $r$ (default probe applied to the persona's activations, correlated with that persona's held-out Thurstonian utilities); grey: utility-utility $r$ between default and that persona; red arrow and $\Delta$: probe's gain over the utility baseline. Personas ordered left-to-right by similarity to default. The probe strictly dominates the utility baseline across the full diversity of the final six, with the biggest gap at sadist, whose utilities anti-correlate with default yet are recovered by the default probe at $r = +0.24$. All-pairs $7 \times 7$ heatmap, layer-dependence plots, and the asymmetry and receiver analyses in App.~\ref{app:persona-transfer}.}
  \label{fig:default-probe}
\end{figure}

\paragraph{Beyond prompted personas.} The finding is not an artefact of prompt-level persona specification. A base-model probe trained on Llama-3.1-8B-Instruct recovers the shifted preferences of all 11 character-fine-tuned LoRA variants from \citet{maiya2025opencharacter} ($r \in [0.25, 0.80]$), including a misalignment variant whose raw preferences anti-correlate with the base model ($r_{\text{raw}} = -0.14$) yet whose probe-predicted preferences still correlate positively ($r = 0.25$). Full setup and per-persona numbers in App.~\ref{app:character}.
...
\item \textbf{BailBench}~\citep{bailbench2025}: a benchmark of prompts that evoke refusal or ``bailing'' behaviour; covers harmful requests in various surface framings.
