...
\begin{figure}[H]
  ...
  \caption{\textbf{Full $7 \times 7$ transfer and two controls.} Rows: probe persona. Cols: eval persona. \emph{Left:} probe transfer $r(\hat u, u_E)$ at eot / layer 32. \emph{Middle:} transfer $r$ minus the utility-utility correlation on the same pair --- how much the probe beats the naïve ``predict via behavioural similarity'' baseline. Every off-diagonal cell is positive. \emph{Right:} partial $r(\hat u, u_T \mid u_E)$ --- the probe's train-specific structure that survives regressing out the eval signal; positive means the probe encodes train information not already in eval, i.e., persona-specific rather than merely tracking shared ``general task-goodness''. Diagonals of the middle and right panels are not informative and are masked.}
