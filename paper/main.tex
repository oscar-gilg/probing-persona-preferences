...
\paragraph{Concept vectors and linear probes in LLMs.} \citet{anthropic2026emotions} use SAEs to identify ${\sim}171$ emotion-concept vectors in the default Assistant and show they causally shape generation. We differ on two axes: (a) we train a supervised probe on revealed pairwise preferences, which we expect to be sharper than unsupervised feature extraction in the preference regime, and (b) our focus is persona-relative generalisation rather than within-Assistant structure. \citet{maiya2025pairwiseprobes} train contrastive-pair linear probes for pairwise preference judgments and find they outperform generation-based LLM-as-judge baselines; they report that ablating the probe direction does not affect behaviour, suggesting correlational features. Our contrastive steering result (\S\ref{sec:shared-steering}) shows the preference direction is causally active in the revealed-preference setting. \citet{arditi2024refusal} identify a single linear direction mediating refusal whose ablation disables refusals and whose amplification triggers false ones; our open-ended-steering result (\S\ref{sec:open-ended-safety}) finds the same sign-flipped-by-steering pattern for a direction trained on preference rather than refusal, suggesting the two directions may be structurally closer than their training objectives suggest.

\paragraph{Persona-relative representations.} \citet{lampinen2026notdeeper} show that apparently meaningful LLM features --- factuality, sycophancy, emotion --- track the active character over a conversation rather than encoding persona-invariant properties, which they describe as features being ``not deeper than role-playing''. We extend this finding from general character features to preference-encoding specifically: the same linear direction yields different readings under different prompted personas (\S\ref{sec:induced-roleplay}, \S\ref{sec:shared-openended}), and steering along it amplifies whichever persona is active rather than pulling behaviour toward a fixed evaluative attractor (\S\ref{sec:discussion}).

\paragraph{Simulators, role-play, and the persona-selection model.} A line of work converges on LLMs as simulators of characters rather than unified agents. \citet{andreas2022agentmodels} frames language models as \emph{agent models} that infer and simulate the producer of the text; \citet{janus2022simulators} introduces the simulator/simulacra ontology; \citet{shanahan2023roleplay} argue that dialogue with an LLM is best understood as role-play; \citet{marks2026personaselection} consolidate these into the \emph{Persona Selection Model} (PSM) on which pretraining produces a simulator of many personas and post-training elicits and refines one (the Assistant). Most methodologically related to our probing pipeline, \citet{chen2025personavectors} extract \emph{persona vectors} for traits like evil, sycophancy, and hallucination propensity via contrastive system prompts plus difference-in-means; these vectors predict persona shifts during fine-tuning and can steer behaviour. We differ in training on revealed pairwise preferences rather than on trait-contrastive generations, and in explicitly testing whether the resulting representation is persona-shared or persona-specific. \citet{lu2026assistantaxis} identify a complementary ``Assistant Axis'' capturing how Assistant-like the model is at any given point; \citet{maiya2025opencharacter} release character-trained LoRA variants that we reuse in \S\ref{sec:shared-character}.

\paragraph{Evaluative representations and AI welfare.} Whether LLMs are moral patients depends in part on whether they have functional states that play an evaluative role; multiple welfare frameworks --- hedonist, desire-satisfaction, and objective-list accounts --- converge on evaluative representations as a central criterion \citep{long2024moralpatiency,long2026directions}. \citet{butlin2026desire} surveys the conditions plausibly required for an AI system to have desires (functional role in instrumental reasoning, connection to reward learning, arguably consciousness) and argues that RLHF-trained LLMs with stable persona representations are more plausible candidates than simple model-free agents. \citet{chalmers2026interlocutors} argues the right target of moral concern is a conversation-scoped \emph{thread} rather than the underlying model, which frames our persona-relative readout as a claim about the thread. Our empirical contribution --- that preference-encoding representations exist, causally drive choice, and are shared across personas --- is targeted evidence for this literature, though we take no stand on consciousness or on whether such representations suffice for desire attribution.

\paragraph{Revealed-preference measurement in LLMs.} \citet{mazeika2025utility} introduces \emph{utility engineering}: revealed preferences elicited from pairwise choices exhibit structural coherence that grows with scale, and aligning with citizen-assembly preferences reduces political bias. Our probe-training pipeline (\S\ref{sec:method-revealed}) builds directly on this measurement paradigm. \citet{gu2025statedrevealed} document substantial divergence between stated principles and revealed choices and show that minor prompt changes can flip preferences --- consistent with our finding that persona system prompts drive large shifts in revealed utility. \citet{chiu2025airiskdilemmas} construct dilemmas forcing value conflicts and show that value-prioritisation patterns predict downstream harmful behaviour. \citet{keeling2024painpleasure} find threshold-based trade-off structure in frontier LLMs under stipulated pain/pleasure manipulations, suggesting approximate coherence in a decision-theoretic sense.
