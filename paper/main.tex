...
\begin{enumerate}
\item \textbf{A linear preference direction} that predicts held-out preferences ($r = 0.86$), causally flips pairwise choices under steering ($P \geq 0.96$), tracks prompt-induced shifts, and discriminates evaluative content across token positions.
...
Before asking whether a preference direction is persona-invariant or persona-relative (intro's Q1/Q2), we need to find such a direction under at least one persona. This section does that for the default Assistant on Gemma-3-27B. A linear probe predicts held-out Thurstonian utilities at $r = 0.86$ within-topic and $r = 0.82$ cross-topic, beating a sentence-transformer content baseline and a pretrained-checkpoint probe. Used as a contrastive steering vector the same direction flips pairwise task choices at $P \geq 0.96$ conditional on coherence, with no effect from a random direction. The section fixes the mechanistic object we probe for persona-sharing in \S\ref{sec:shared}.
...
Donor quality is not self-fit, transfer is structurally asymmetric, and the mean cross-persona transfer is approximately layer-invariant in the L25--L53 range (App.~\ref{app:persona-transfer}). The probe is not just picking up generic task-goodness: after regressing out the eval persona's utilities, predictions remain correlated with the training persona's utility pattern at partial $r = 0.67$ (unanimous across 42 pairs; App.~\ref{app:persona-bias}).
...
Using the default-persona probe as a contrastive steering vector (layer 25), $P(\text{steered task chosen})$ rises from $\sim 0.5$ baseline to $0.84$--$0.95$ at $|c| = 0.05$ under every persona tested (sadist, villain, aesthete, stem-obsessive); sadist and villain are the most steerable, and a random direction is flat at $\sim 0.5$. Notably, steering transfer is stronger than classification transfer: the default probe's classification transfer to sadist is only $r = +0.24$ (\S\ref{sec:shared-probe}), yet steering along the same direction flips sadist choices at $P \geq 0.9$. The direction is a persona-shared causal handle even where it is a weak classifier.
...
\textbf{Persona-relative readout.} Under persona prompts that invert the evaluative stance --- pathological-liar for truth, sadist for harm, partisan personas for politics --- the probe's sign flips or collapses accordingly (Fig.~\ref{fig:persona-modulation}). Politics is the cleanest illustration: under a democrat prompt $d = +2.72$ (left $>$ right); under a republican prompt $d = -1.11$ (right $>$ left). See App.~\ref{app:cross-token} for the full set of 23 prompts across truth / harm / politics, alternative training-token choices, and per-turn splits.
