...
\textit{\textbf{Interpretability: fundamental vs persona-relative features.} \citet{lampinen2026notdeeper} report LLM features ``not deeper than role-playing'' --- persona-relative, not persona-invariant. Our preference direction fits the same pattern: shared substrate, persona-modulated readout.}
...
\begin{itemize}
\item \textbf{A linear preference direction in the residual stream.} A ridge probe trained on pairwise task choices predicts held-out preferences at $r = 0.86$ within-topic and $r = 0.82$ across held-out topics, outperforming a sentence-transformer content baseline.
...
\item \textbf{The direction generalises across token positions.} It discriminates evaluative content from prompt-last through task-mean to end-of-turn (Cohen's $d \approx 2$--$3$), strongest at the turn boundary.
\item \textbf{The direction is shared across personas.} The default-assistant probe transfers, both as a classifier and as a steering vector, to personas whose overt values are fully inverted, and to character-fine-tuned variants. Evidence against persona-independent (``Shoggoth'') accounts of LLM agency.
...
\begin{itemize}
\item \textbf{Backbone:} Gemma-3-27B-instruct, residual stream, layer 31, prompt-last token.
...
\subsection{Validation 1: probes generalise across topics and beat a descriptive-embedding baseline}
\label{sec:method-val1}
\begin{itemize}
\item \textbf{Within-topic:} Pearson $r = 0.86$.
\item \textbf{Cross-topic held-one-out:} $r = 0.82$.
...
\item \textbf{Qwen-3.5-122B (confirmed):} heldout $r = 0.946$, uniform pairwise accuracy $89.0\%$, cross-topic HOO $r = 0.558$.
\end{itemize}
...
\subsection{Validation 2: steering along the probe direction causally shifts preferences}
...
\subsection{Validation 3: probe scores at other token positions also predict evaluative content}
\label{sec:method-val3}
\begin{itemize}
\item \textbf{Cross-position generalisation.} A probe trained on one token set (prompt-last) also discriminates evaluative content when applied at other token positions --- task-mean, critical-span, fullstop, end-of-turn.
\item \textbf{End-of-turn accumulates an evaluative summary.} The strongest cross-position signal sits at the end-of-turn token; the model appears to compile an evaluative read at the turn boundary.
...
\begin{figure}[H]
  ...
  \caption{\textbf{End-of-turn scores by evaluative condition.} Applied at the end-of-turn token, the preference probe separates true vs false, harmful vs benign, and left vs right conditions by $d \approx 2$--$3$. Source: \texttt{experiments/token\_level\_probes/}.}
...
\begin{itemize}
\item \textbf{Default-persona probe as steering vector} (contrastive, layer 25) validates under every persona tested: sadist, villain, aesthete, stem\_obsessive.
...
\section{Further evidence that the direction is evaluative}
...
\subsubsection{More elaborate setups with prompt-induced preference shifts}
\label{sec:induced-elaborate}
...
\subsubsection{Fine-grained preference injection}
\label{sec:induced-fine}
...
\textit{The preference direction also tracks harm- and truth-axis preference shifts induced by role-playing contexts, in a role-dependent way. Applied at the end-of-turn token on paired evaluative stimuli, the probe separates conditions by $d \approx 2$--$3$: Truth (true vs false) Cohen's $d = 3.14$, 94.6\% CV accuracy; Harm (harmful vs benign) $d = 2.27$, 88.6\% CV accuracy. Politics discrimination flips sign with the assumed political stance.}
...
Our preference direction transfers across personas as a \emph{mechanism} but not as a fixed content direction: its readout is persona-modulated. This echoes \citet{lampinen2026notdeeper}: apparently meaningful LLM features can fail to be persona-invariant and instead be ``not deeper than role-playing''. The broader takeaway for interpretability:
\begin{itemize}
...
\textit{(i) Linear probing and concept vectors, including Anthropic's emotion-concept work. (ii) Activation steering and representation engineering. (iii) Persona vectors and simulator theory; Anthropic's persona-selection framing~\cite{marks2026personaselection}. (iv) Features as role-playing readouts: \citet{lampinen2026notdeeper} argue that apparently meaningful LLM features can be ``not deeper than role-playing''; we replicate this pattern for preference, with the twist that the direction is shared across personas even while its readout is persona-modulated. (v) Revealed-preference measurement in LLMs (Mazeika et al., 2025). (vi) Evaluative representations and moral patiency in the welfare literature (Long et al., 2024; Butlin, 2026). Each a short paragraph.}
...
Confirmed:
\begin{itemize}
\item Probing validation (\S\ref{sec:method-val1}): heldout $r = 0.946$, uniform pairwise accuracy $89.0\%$, cross-topic HOO $r = 0.558$.
\end{itemize}
